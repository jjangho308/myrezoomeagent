...
public class AmqAgentImpl implements AmqAgent, Callable<Integer>, MessageListener {
  ...
  public void recieveMsg(int timeoutTime) throws Exception {
    ...
    /*
     * // Begin to wait for messages. Message consumerMessage = (Message)
     * consumer.receive(this.consumerTimeout);
     * 
     * // Receive the message when it arrives. TextMessage consumerTextMessage = (TextMessage)
     * consumerMessage; System.out.println("Message received: " + consumerTextMessage.getText());
     */
  ...
  public void onMessage(Message consumerMessage) {
    try {
      ...
      job.setJob(MessageWrapperImpl.getInstance().convertMessageToJob(consumerTextMessage.getText()));
      System.out.println("Message received: " + consumerTextMessage.getText());
