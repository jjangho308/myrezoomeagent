...
public class AllTests extends TestSuite {
  ...
  //
  // System.out.println("var publicKey = '" +
  // Base64.getEncoder().encodeToString(clientPublic.getEncoded()) + "';");
  // System.out.println("var privateKey = '" +
  // Base64.getEncoder().encodeToString(clientPrivate.getEncoded()) + "';");
  // System.out.println("var aesKey = '" +
  // Base64.getUrlEncoder().encodeToString(aesKey.getEncoded()) + "';");
  ...
  // String plainText =
  // "택수의 개발 과부하 현상이 심화되면 올해 레쥬메의 3% 성장 달성도 힘겨울 수밖에 없다. 무엇보다 우리 개발진이 구조적 위기에 직면하고 있다. 제조업은 좋은 일자리의
  // 원천이고 성장을 떠받치는 근간이다. 제조업의 체질 개선과 구조개혁, 규제철폐로 기업의 혁신을 촉진할 수 있는 산업정책의 근본적인 재정립이 시급하다. 그런 정책이
  // 안보인다.이런 현상이 심화되면 올해 한국 경제의 3% 성장 달성도 힘겨울 수밖에 없다. 무엇보다 우리 제조업이 구조적 위기에 직면하고 있다. 제조업은 좋은 일자리의
  // 원천이고 성장을 떠받치는 근간이다. 제조업의 체질 개선과 구조개혁, 규제철폐로 기업의 혁신을 촉진할 수 있는 산업정책의 근본적인 재정립이 시급하다. 그런 정책이
  // 안보인다.이런 현상이 심화되면 올해 한국 경제의 3% 성장 달성도 힘겨울 수밖에 없다. 무엇보다 우리 제조업이 구조적 위기에 직면하고 있다. 제조업은 좋은 일자리의
  // 원천이고 성장을 떠받치는 근간이다. 제조업의 체질 개선과 구조개혁, 규제철폐로 기업의 혁신을 촉진할 수 있는 산업정책의 근본적인 재정립이 시급하다. 그런 정책이
  // 안보인다.이런 현상이 심화되면 올해 한국 경제의 3% 성장 달성도 힘겨울 수밖에 없다. 무엇보다 우리 제조업이 구조적 위기에 직면하고 있다. 제조업은 좋은 일자리의
  ...
  // + " pkey : 'pkey',\r\n" + " require : ['12060991'],\r\n"
  // + " records : [{hashed: \"f1b9d789ac9ef7343b03c79d331230d7aa99be4b035c91c056bddd750da7dbfc\",
  // subID:\"RCOGC0009\", txid:\"sdfsdfsdf\"},{hashed: \"dddd\", subID:\"RCOGC0008\",
  // txid:\"234234234234\"}]\r\n"
  // + " }\r\n" + "}";
  //
  // String aesKey = ManagerProvider.crypto().generateAES();
  ...
  // String publicKey = ManagerProvider.key().getPubKeyStr("AGENT_CERT");
  //
  // String encryptedAMQFullMessage = ManagerProvider.crypto().encryptRSA(amqFullMessage,
  // publicKey);
  // System.out.println("encrypted AMQ Full message : " + encryptedAMQFullMessage);
  //
  // String decrytedAMQFullMessage = ManagerProvider.crypto().decryptRSA(encryptedAMQFullMessage,
  // privateKey);
  ...
  // *
  // * InitialEvent event = InitialEvent.RUNTIME; try { ServiceInitializer.initialize(event); }
  // catch
